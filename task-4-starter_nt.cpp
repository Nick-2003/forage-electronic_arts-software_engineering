...
public:
    ...
    void remove_item(int item_index) {
        ...
        // Remove item visibility from inventory if quantity = 0
        if (item->get_quantity() == 0) { // Check if quantity is 0
            for (int i = item_index; i < item_count - 1; i++) { // Shift items to the left
                items[i] = items[i + 1];
            }
            item_count--; // Decrement item count by 1
        }
    }
